class AutopilotConfig {
  ...
  factory AutopilotConfig.fromJson(Map<String, dynamic> json) =>
      AutopilotConfig(
        salaryThreshold: (json['salary_threshold'] as num).toDouble(),
        incomeType:
            _incomeTypeFromString(json['income_type'] as String? ?? 'monthly'),
      );

  Map<String, dynamic> toJson() => {
        'salary_threshold': salaryThreshold,
        'income_type': incomeType.name,
      };

  final double salaryThreshold;
  final IncomeType incomeType;

  static IncomeType _incomeTypeFromString(String s) =>
      s.toLowerCase() == 'gig' ? IncomeType.gig : IncomeType.monthly;
}
