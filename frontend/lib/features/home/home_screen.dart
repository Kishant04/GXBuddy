...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  void initState() {
    super.initState();
    // Listen for dashboard changes to show alerts once
  ...
  void _setupListeners() {
    ref.listenManual(
      ...
      (previous, next) {
        final dashboard = next.valueOrNull;
        ...
        if (alerts.isNotEmpty) {
          ...
          final eventId = 'alert_${firstAlert.id}';

          if (ref.read(notificationServiceProvider).shouldShow(eventId)) {
            debugPrint(
                '[UI] Showing new alert notification: ${firstAlert.message}');
            _showToast(firstAlert.message, color: GXColors.warning);
          }
        }
      },
  ...
  Widget _buildDashboard(DashboardModel d, HomeUiState uiState) {
    ...
    return Column(
      ...
      children: [
        // Mascot
        MascotStatusCard(
          mascotState: uiState.mascotOverride?.state ?? d.mascot.state,
          moodLine: uiState.mascotOverride?.moodLine ?? d.mascot.moodLine,
        ...
        DemoTriggerPanel(
          loading: uiState.pendingAction,
          onSpendFood: () {
            debugPrint('[DEMO] Clicked Spend RM50 Food');
            _fireSpend(amount: 50, merchant: 'GrabFood', category: 'FOOD');
          },
          onSpendShopping: () {
            debugPrint('[DEMO] Clicked Spend RM100 Shopping');
            setState(() => _showHighRisk = true);
            _fireSpend(amount: 100, merchant: 'Shopee', category: 'SHOPPING');
          },
          onReceiveSalary: () {
            debugPrint('[DEMO] Clicked Receive Salary');
            _fireReceiveSalary();
          },
          onSave: () {
            debugPrint('[DEMO] Clicked Save RM10');
            _fireSpend(
                amount: 10, merchant: 'Manual Save', category: 'SAVINGS');
          },
        ),
      ],
    );
