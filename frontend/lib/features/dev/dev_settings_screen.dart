...
class _DevSettingsScreenState extends ConsumerState<DevSettingsScreen> {
  ...
  late final TextEditingController _resetKeyCtrl;
  ...
  void initState() {
    ...
    _resetKeyCtrl = TextEditingController(text: store.demoResetKey ?? '');
  ...
  Future<void> _testConnection() async {
    final effectiveUrl = _effectiveApiUrl;
    final testPath = Endpoints.authHealth;
    final fullUrl = '$effectiveUrl$testPath';

    setState(() {
      _connLoading = true;
      _connStatus = 'Attempting GET: $fullUrl\n(Dio BaseUrl: $effectiveUrl)';
    });
    ...
    try {
      ...
      final res = await dio.get(testPath);
      setState(() {
        _connOk = res.statusCode == 200;
        _connStatus = '✓  ${res.statusCode} OK\n'
            'URL: $fullUrl\n'
            'Data: ${res.data}';
      });
    } on DioException catch (e) {
      String details = 'DioError: ${e.type.name}';
      if (e.message != null) details += '\nMessage: ${e.message}';
      if (e.response != null) {
        details += '\nStatus: ${e.response?.statusCode}';
        details += '\nResponse: ${e.response?.data}';
      } else {
        details += '\n\n⚠️ No HTTP response received.\n'
            'Possible causes:\n'
            '1. Backend not running on $effectiveUrl\n'
            '2. CORS blocking (Check Browser Console)\n'
            '3. Wrong port or firewall\n'
            '4. Stale/Wrong saved URL in SharedPreferences';
      }
      ...
      setState(() {
        _connOk = false;
        _connStatus = '✗  Unexpected Error: $e\nURL: $fullUrl';
      });
    } finally {
  ...
  Future<void> _resetConnectionSettings() async {
    ...
    await store.clearMockMode();

    if (mounted) {
      setState(() {
        _apiUrlCtrl.text = '';
        _wsUrlCtrl.text = '';
        _mockMode = AppConfig.useMockData;
        _connStatus = 'Connection settings reset to defaults.\n'
            'Effective API: ${AppConfig.apiBaseUrl}\n'
            'Effective WS: ${AppConfig.wsUrl}';
        _connOk = true;
      });
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Connection overrides cleared.')),
      );
    }
  }
  ...
  Future<void> _clearAllSettings() async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: GXColors.bgCard,
        title: const Text('Clear All Dev Settings?',
            style: TextStyle(color: Colors.white)),
        content: const Text(
          'This will clear your Token, User ID, and all URL overrides. Continue?',
    ...
    if (mounted) {
      setState(() {
        ...
        _tokenCtrl.clear();
        _mockMode = AppConfig.useMockData;
        _connStatus = 'All settings wiped.';
        _authStatus = null;
      });
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _saveSettings() async {
    ...
    final token = _tokenCtrl.text.trim();
    final resetKey = _resetKeyCtrl.text.trim();
    ...
    } else if (_tokenCtrl.text.isEmpty) {
      await store.clearToken();
    }

    // Reset Key
    if (resetKey.isNotEmpty) {
      await store.setDemoResetKey(resetKey);
    } else {
      await store.clearDemoResetKey();
    }
  ...
  Future<void> _resetDemo() async {
    ...
    try {
      await ref.read(repositoryProvider).resetDemoData(userId: userId);

      // Clear cached squad ID to force fresh discovery
      await ref.read(authTokenStoreProvider).setSquadId('');
      ref.read(squadIdStateProvider.notifier).state = null;

      // Invalidate everything to show the reset state
      ref.invalidate(homeDashboardProvider);
      ref.invalidate(transactionsProvider);
      ref.invalidate(pocketsAsyncProvider);
      ref.invalidate(squadNotifierProvider);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            backgroundColor: Color(0xFF14053A),
            content: Text('Demo data reset successful! Baseline restored.',
                style: TextStyle(color: GXColors.success)),
          ),
        );
      }
    } catch (e) {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          ...
                          Expanded(
                            child: _ActionButton(
                              ...
                              onTap: _resetConnectionSettings,
                            ),
                          ),
                        ],
                      ),
                      ...
                      _Card(
                        child: Column(
                          children: [
                            ...
                            _Divider(),
                            _Field(
                              label: 'Demo Reset Key',
                              controller: _resetKeyCtrl,
                              hint: 'local-demo-reset-key',
                              hint2: 'X-Demo-Reset-Key header value',
                            ),
                            ],
                            ),
                            ),
                            const SizedBox(height: 8),
                            const _HintText(
                            'JWT is optional for demo mode. Main demo uses saved User ID.',
                            ),
                            const SizedBox(height: 20),
                      ...
                      const SizedBox(height: 12),
                      _ActionButton(
                        label: 'Clear All Dev Settings',
                        icon: Icons.delete_forever_rounded,
                        onTap: _clearAllSettings,
                        danger: true,
                      ),
                      const SizedBox(height: 20),
                      // ── Quick ref ─────────────────────────────────────────
...
class _ActionButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final c = color ?? (danger ? GXColors.danger : GXColors.violet);
