...
class ProfileScreen extends ConsumerWidget {
              ...
              GXCard(
                ...
                child: Column(
                  children: [
                    ...
                    Text(user.name.isEmpty ? 'Loading...' : user.name,
                        style: const TextStyle(
                            fontSize: 19,
                            fontWeight: FontWeight.w800,
                            color: GXColors.textWhite,
                    ...
                    _StatsRow(
                        ...
                        salaryThreshold: user.salaryThreshold,
              ...
              _Section(
                ...
                children: [
                  _SettingRow(
                    ...
                    label: 'Freeze card',
                    danger: true,
                    value: user.cardFrozen ?? false,
                    onToggle: notifier.toggleFreezeCard,
                  ),
                  _SettingRow(
                      icon: '📉',
                      label: 'Spending limit',
                      chevron: true,
                      right: user.weeklySpendingLimit != null &&
                              user.weeklySpendingLimit! > 0
                          ? 'RM${user.weeklySpendingLimit!.toStringAsFixed(0)}/wk'
                          : 'Set limit',
                      onToggle: () => _showSpendingLimitDialog(context, ref)),
                  _SettingRow(
                      icon: '🚨',
                      label: 'Scam alert support',
                      chevron: true,
                      onToggle: () => _showScamAlertSupport(context)),
                ],
              ),
              const SizedBox(height: 14),
  ...
  void _showScamAlertSupport(BuildContext context) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: const Column(
          ...
          children: [
            Text(
              'If you suspect a scam or unauthorized transaction, you can:',
              style: TextStyle(color: GXColors.textSoft, fontSize: 13.5),
            ),
            SizedBox(height: 16),
            Text('• Call 997 (National Scam Response Centre)',
                style: TextStyle(
                    color: GXColors.textWhite,
                    fontWeight: FontWeight.w600,
                    height: 1.5)),
            Text('• Use the "Kill Switch" in GXBank app',
                style: TextStyle(
                    color: GXColors.textWhite,
                    fontWeight: FontWeight.w600,
                    height: 1.5)),
            Text('• Freeze your card immediately in Profile',
                style: TextStyle(
                    color: GXColors.textWhite,
                    fontWeight: FontWeight.w600,
                    height: 1.5)),
        ...
        actions: [
          ...
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: const Text('Call GX Support',
  ...
  void _showSpendingLimitDialog(BuildContext context, WidgetRef ref) {
    final ctrl = TextEditingController();
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF1F0A4A),
        title: const Text('Weekly Spending Limit',
            style: TextStyle(color: GXColors.textWhite)),
        content: TextField(
          controller: ctrl,
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              ...
              if (val > 0) {
                ref.read(profileProvider.notifier).updateSpendingLimit(val);
                Navigator.pop(ctx);
              }
            },
  ...
  Widget build(BuildContext context) => ClipRRect(
        ...
        child: Row(
          children: [
            ...
            _Stat(
                value: 'RM${salaryThreshold.toStringAsFixed(0)}',
                label: 'Threshold'),
            Container(width: 1, height: 60, color: GXColors.border),
