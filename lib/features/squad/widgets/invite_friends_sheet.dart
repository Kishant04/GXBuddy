...
class InviteFriendsSheet extends StatelessWidget {
  ...
  /// Live squad data. When null, demo values are used.
  final SquadModel? squad;

  String get _inviteCode => squad?.inviteCode ?? 'GXBUDDY-BRKNOMORE';

  List<String> get _memberNames {
    if (squad == null) {
      return ['Aiman (You)', 'Mei', 'Kumar', 'Sarah'];
    }
    return squad!.members.map((m) {
      return m.isYou ? '${m.displayName} (You)' : m.displayName;
    }).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final emptySlots = (_maxMembers - members.length).clamp(0, _maxMembers);
    ...
    return Container(
      ...
      padding: EdgeInsets.fromLTRB(
          20, 18, 20, MediaQuery.of(context).padding.bottom + 28),
      child: Column(
        ...
        children: [
          ...
          const Text('Grow stronger together. Up to 5 members per squad.',
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: GXColors.violet.withValues(alpha: 0.27)),
              ),
              child: Row(
                children: [
                  Expanded(
          ...
          ...List.generate(
            ...
            (i) => Padding(
              padding: const EdgeInsets.only(bottom: 8),
              child: _FriendSlot(
                  name: 'Empty slot', index: members.length + i, empty: true),
            ),
          ),
          const SizedBox(height: 16),
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(14),
              border:
                  Border.all(color: GXColors.success.withValues(alpha: 0.20)),
            ),
            child: const Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    ...
                    style: TextStyle(
                        fontSize: 12, color: GXColors.textSoft, height: 1.4),
                  ),
                ),
              ],
            ),
...
class _FriendSlot extends StatelessWidget {
  const _FriendSlot(
      {required this.name, required this.index, this.empty = false});
  final String name;
  final int index;
  final bool empty;
            ...
            child: Center(
              child: Text(
                ...
                style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.w800,
                    color: empty ? GXColors.textMute : color),
              ),
            ),
          ),
