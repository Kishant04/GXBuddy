...
class _HighRiskModalState extends State<HighRiskModal>
  ...
  void initState() {
    ...
    _slideAnim = Tween(begin: const Offset(0, 1), end: Offset.zero).animate(
      CurvedAnimation(parent: _slideCtrl, curve: const Cubic(0.16, 0.84, 0.32, 1)),
    );
    _slideCtrl.forward();
    ...
    _scoreTimer = Timer.periodic(const Duration(milliseconds: 22), (t) {
      score += 4;
      if (score >= 82) { score = 82; t.cancel(); }
      if (mounted) setState(() => _riskScore = score);
    });

    _countdownTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted) return;
      setState(() { if (_secsLeft > 0) _secsLeft--; });
    });
  }
            ...
            child: SlideTransition(
              ...
              child: Container(
                decoration: const BoxDecoration(
                  ...
                  boxShadow: [BoxShadow(color: Color(0xB3000000), blurRadius: 80, offset: Offset(0, -30))],
                ),
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      // Countdown ring
                      _CountdownRing(secsLeft: _secsLeft, ringPct: ringPct, ringColor: ringColor),
                      const SizedBox(height: 8),
                      const Text(
                        '· Pause Before You Spend ·',
                        style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w800,
                            color: Color(0xFFFF7DA1), letterSpacing: 0.2),
                      ),
                      const SizedBox(height: 8),
                      RichText(
                        textAlign: TextAlign.center,
                        text: const TextSpan(children: [
                          TextSpan(text: 'RM', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: GXColors.textSoft)),
                          TextSpan(text: '100.00', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: GXColors.textWhite, letterSpacing: -0.035)),
                          TextSpan(text: ' · Shopee', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: GXColors.textSoft)),
                        ]),
                      ),
                      const SizedBox(height: 8),
                      const Padding(
...
class _CountdownRing extends StatelessWidget {
  const _CountdownRing({required this.secsLeft, required this.ringPct, required this.ringColor});
  final int secsLeft;
            ...
            Container(
              decoration: BoxDecoration(
                ...
                gradient: RadialGradient(
                  colors: [ringColor.withValues(alpha: 0.20), Colors.transparent],
                ),
              ),
            ),
            // Ring
              ...
              children: [
                Text(
                  ...
                  style: const TextStyle(
                    ...
                    color: GXColors.textWhite, letterSpacing: -0.04, height: 1,
                  ),
                ),
                const Text(
                  'BREATHE',
                  style: TextStyle(fontSize: 9, color: GXColors.textMute, letterSpacing: 0.16, fontWeight: FontWeight.w700),
                ),
              ],
            ),
          ],
          ...
          children: [
            Row(
              children: [
                ...
                const Expanded(child: Text('Risk score', style: TextStyle(fontSize: 12, color: GXColors.textSoft, fontWeight: FontWeight.w600))),
                Text(
                  '$riskScore',
                  style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: Color(0xFFFF7DA1), letterSpacing: -0.025),
                ),
                const Text(' / 100', style: TextStyle(fontSize: 13, color: GXColors.textMute)),
              ],
            ),
              ...
              builder: (_, c) => Container(
                ...
                child: FractionallySizedBox(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(
                        colors: [Color(0xFFFBB347), GXColors.pink, GXColors.danger],
                      ),
                      borderRadius: BorderRadius.circular(99),
                      boxShadow: [BoxShadow(color: GXColors.danger.withValues(alpha: 0.40), blurRadius: 14)],
                    ),
                  ),
                ),
              ),
            ...
            const Row(
              ...
              children: [
                Text('SAFE', style: TextStyle(fontSize: 9.5, color: GXColors.textMute, fontWeight: FontWeight.w600, letterSpacing: 0.06)),
                Text('WATCH', style: TextStyle(fontSize: 9.5, color: GXColors.textMute, fontWeight: FontWeight.w600, letterSpacing: 0.06)),
                Text('HIGH RISK', style: TextStyle(fontSize: 9.5, color: GXColors.textMute, fontWeight: FontWeight.w600, letterSpacing: 0.06)),
              ],
            ),
            const SizedBox(height: 12),
            Wrap(
              spacing: 6, runSpacing: 6,
              children: _tags.map((t) => _TagChip(label: t.label, color: t.color)).toList(),
            ),
          ],
        ),
      );
...
class _TagChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Text(label, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: GXColors.textWhite)),
      );
}
